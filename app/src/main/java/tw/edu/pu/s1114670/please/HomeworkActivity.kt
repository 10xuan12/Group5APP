...
fun HomeworkScreen() {
    ...
    Column(
        ...
        Row(
            ...
            Button(
                onClick = {
                    val userTasks = hashMapOf(
                        "morningTask" to mapOf(
                            "task" to morningTask.text,
                            "completed" to morningChecked
                        ),
                        "afternoonTask" to mapOf(
                            "task" to afternoonTask.text,
                            "completed" to afternoonChecked
                        ),
                        "nightTask" to mapOf(
                            "task" to nightTask.text,
                            "completed" to nightChecked
                        )
                    )

                    db.collection("users").document("派給小朋友的任務").collection("tasks")
                        .document("dailyTasks")
                        .set(userTasks, SetOptions.merge())
